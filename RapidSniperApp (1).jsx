    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", fontFamily:FD }}>
      <div className="fade-up" style={{ width:"100%", maxWidth:"410px" }}>
        <div style={{ textAlign:"center", marginBottom:"30px" }}>
          ...
          <div style={{ fontSize:"26px", fontWeight:"900", background:C.grad1, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", letterSpacing:"-0.5px" }}>TRADING SNIPER</div>
          <div style={{ color:C.muted, fontSize:"12px", fontWeight:"600", marginTop:"4px", letterSpacing:"2px" }}>TRADING STRATEGY BUILDER</div>
        </div>
    ...
    <div style={{ padding:"40px 24px", maxWidth:"1060px", margin:"0 auto", fontFamily:FD }} className="fade-up">
      <div style={{ textAlign:"center", marginBottom:"44px" }}>
        ...
        <h1 style={{ fontSize:"clamp(26px,5vw,52px)", fontWeight:"900", letterSpacing:"-1px", lineHeight:1.1, marginBottom:"14px" }}>
          ...
          <br/><span style={{ color:C.text, fontSize:"0.62em", fontWeight:"600", letterSpacing:"2px" }}>TRADING STRATEGY BUILDER</span>
        </h1>
        <p style={{ 
...
// MT5 official WebTerminal URL — from metatraderweb.app
// demo_all_servers=1 → all brokers available
// startup_mode=open_demo → opens demo account dialog on launch
// lang=en → English interface
// save_password=off → don't save passwords (security)
// utm_campaign=rapidsniper → tags demo accounts opened from this app
const MT5_URL = "https://metatraderweb.app/trade?demo_all_servers=1&startup_mode=open_demo&lang=en&save_password=off&utm_campaign=rapidsniper";

// TradingView advanced chart embed — dark theme, 15min, EURUSD default
const TV_URL = "https://s.tradingview.com/widgetembed/?symbol=EURUSD&interval=15&theme=dark&style=1&locale=en&timezone=Etc%2FUTC&hidesidetoolbar=0&hidetoptoolbar=0&symboledit=1&saveimage=1&withdateranges=1&showpopupbutton=1&utm_source=rapidsniper&utm_medium=widget&utm_campaign=chart";

function TradingPlatforms() {
  const [activeTab, setActiveTab] = useState("mt5");
  const [mt5Loaded, setMt5Loaded] = useState(false);
  const [tvLoaded, setTvLoaded] = useState(false);

  const tabs = [
    { id:"mt5", label:"MetaTrader 5", icon:"⚡", color:C.purple,  dot:C.purpleHi },
    { id:"tv",  label:"TradingView",  icon:"📊", color:C.green,   dot:C.greenHi  },
  ];
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 58px)", fontFamily:FD }}>
      ...
      <div style={{ padding:"16px 24px 0", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"12px", marginBottom:"14px" }}>
          <div>
            <div style={{ display:"inline-block", padding:"3px 14px", borderRadius:"20px", background:"rgba(139,92,246,0.1)", border:`1px solid ${C.border}`, color:C.purpleHi, fontSize:"11px", fontWeight:"700", letterSpacing:"1px", marginBottom:"8px" }}>📈 LIVE PLATFORMS</div>
            <h2 style={{ fontSize:"24px", fontWeight:"900", letterSpacing:"-0.5px", color:C.text, margin:0 }}>
              Trading <span style={{ color:C.purpleHi }}>Desk</span>
            </h2>
          </div>

          {/* Tab switcher */}
          <div style={{ display:"flex", gap:"6px", background:"rgba(10,4,24,0.85)", border:`1px solid ${C.border}`, borderRadius:"12px", padding:"5px" }}>
            {tabs.map(t=>(
              <button key={t.id} onClick={()=>setActiveTab(t.id)} className="btn-hover"
                style={{ fontFamily:FD, fontWeight:"700", fontSize:"12px", padding:"9px 20px", borderRadius:"9px", cursor:"pointer", border:"none",
                ...
                {activeTab===t.id && <span style={{ width:"6px", height:"6px", borderRadius:"50%", background:"#fff", opacity:0.7 }}/>}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* ── Embed area ─────────────────────────────────────────────────────── */}
      <div style={{ flex:1, padding:"0 24px 20px", display:"flex", flexDirection:"column", minHeight:0 }}>

        {/* ── MetaTrader 5 ── */}
        <div style={{ display: activeTab==="mt5" ? "flex" : "none", flex:1, flexDirection:"column", minHeight:0, ...gc(C.purple), overflow:"hidden" }}>
          {/* Status bar */}
          <div style={{ padding:"11px 18px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:"12px", flexShrink:0, flexWrap:"wrap" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
              <div style={{ width:"9px", height:"9px", borderRadius:"50%", background: mt5Loaded ? C.greenHi : C.amber, boxShadow:`0 0 7px ${mt5Loaded ? C.green : C.amber}`, transition:"background 0.4s" }}/>
              <span style={{ fontSize:"13px", fontWeight:"700", color:C.purpleHi }}>MetaTrader 5 — Official WebTerminal</span>
            </div>
            <div style={{ display:"flex", gap:"8px", marginLeft:"auto", flexWrap:"wrap" }}>
              <span style={{ fontSize:"11px", color:C.muted, padding:"3px 10px", border:`1px solid ${C.border}`, borderRadius:"6px" }}>All Brokers · Demo + Live</span>
              <span style={{ fontSize:"11px", color:C.green, padding:"3px 10px", border:`1px solid ${C.green}40`, borderRadius:"6px", background:`${C.green}0d` }}>🔒 metatraderweb.app</span>
            </div>
          </div>

          {/* Loading overlay */}
          {!mt5Loaded && (
            <div style={{ position:"absolute", inset:0, zIndex:5, background:"rgba(8,4,18,0.92)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"16px", pointerEvents:"none" }}>
              <div style={{ fontSize:"36px", animation:"float 2s ease-in-out infinite" }}>⚡</div>
              <div style={{ fontSize:"14px", fontWeight:"700", color:C.purpleHi }}>Loading MT5 Web Terminal…</div>
              <div style={{ fontSize:"12px", color:C.muted }}>Connecting to metatraderweb.app</div>
            </div>
          )}

          {/* Official MT5 WebTerminal iframe — metatraderweb.app API */}
          <div style={{ flex:1, minHeight:0, position:"relative" }}>
            <iframe
              src={MT5_URL}
              allowFullScreen={true}
              onLoad={()=>setMt5Loaded(true)}
              style={{ width:"100%", height:"100%", border:"none", display:"block" }}
              title="MetaTrader 5 Web Terminal — Rapid Sniper"
            />
          </div>

          {/* Info footer */}
          <div style={{ padding:"10px 18px", borderTop:`1px solid ${C.border}`, fontSize:"11px", color:C.muted, flexShrink:0, display:"flex", alignItems:"center", gap:"12px", flexWrap:"wrap" }}>
            <span>⚡ Open a demo account directly in the terminal — all brokers available. Use your existing live account credentials to connect.</span>
            <div style={{ display:"flex", gap:"10px", marginLeft:"auto" }}>
              <a href="https://metatraderweb.app/trade" target="_blank" rel="noopener noreferrer"
                style={{ color:C.purpleHi, fontWeight:"700", textDecoration:"none" }}>Open Full Screen ↗</a>
              <a href="https://www.metatrader5.com" target="_blank" rel="noopener noreferrer"
                style={{ color:C.muted, fontWeight:"600", textDecoration:"none" }}>MT5 Website ↗</a>
            </div>
          </div>
        </div>

        {/* ── TradingView ── */}
        <div style={{ display: activeTab==="tv" ? "flex" : "none", flex:1, flexDirection:"column", minHeight:0, ...gc(C.green), overflow:"hidden" }}>
          {/* Status bar */}
          <div style={{ padding:"11px 18px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:"12px", flexShrink:0, flexWrap:"wrap" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
              <div style={{ width:"9px", height:"9px", borderRadius:"50%", background: tvLoaded ? C.greenHi : C.amber, boxShadow:`0 0 7px ${tvLoaded ? C.green : C.amber}`, transition:"background 0.4s" }}/>
              <span style={{ fontSize:"13px", fontWeight:"700", color:C.greenHi }}>TradingView — Advanced Charts</span>
            </div>
            <div style={{ display:"flex", gap:"8px", marginLeft:"auto", flexWrap:"wrap" }}>
              <span style={{ fontSize:"11px", color:C.muted, padding:"3px 10px", border:`1px solid ${C.border}`, borderRadius:"6px" }}>EURUSD · 15min · Dark</span>
              <span style={{ fontSize:"11px", color:C.green, padding:"3px 10px", border:`1px solid ${C.green}40`, borderRadius:"6px", background:`${C.green}0d` }}>Free · No Login</span>
            </div>
          </div>
          ...
          <div style={{ flex:1, minHeight:0, position:"relative" }}>
            <iframe
              ...
              style={{ width:"100%", height:"100%", border:"none", display:"block" }}
              title="TradingView Chart — Rapid Sniper"
            />
          </div>
          ...
          <div style={{ padding:"10px 18px", borderTop:`1px solid ${C.border}`, fontSize:"11px", color:C.muted, flexShrink:0, display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap" }}>
            <span>💡 Use the symbol search bar inside the chart to switch to any of your watchlist pairs — XAU/USD, GBP/JPY, NAS100 and more.</span>
            <a href="https://www.tradingview.com/chart/" target="_blank" rel="noopener noreferrer"
              style={{ marginLeft:"auto", color:C.greenHi, fontWeight:"700", textDecoration:"none" }}>Open in TradingView ↗</a>
          </div>
        </div>
